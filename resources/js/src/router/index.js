...
{
    // Validar si es necesaria autenticación
    if(to.meta.roles){
        if(!auth.state.status.loggedIn){
            window.location.href = '/auth/login';
        }else{
            auth.actions.checkLogin();
        }

        const rutaRoles = to.meta.roles;
        const usuarioRoles = auth.state.roles;
        const permitido = rutaRoles.filter(element => usuarioRoles.includes(element));
        if(rutaRoles && permitido.length < 1){
            router.push('/');
        }
    }
